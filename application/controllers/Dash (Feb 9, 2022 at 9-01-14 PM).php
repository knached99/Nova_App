...
class Dash extends CI_Controller {
	...
	{ $this->load->model('Notifications_Model');
		...
		if($release->release_artist_main !== $_SESSION['artist_account_name_artist']){
			redirect('https://my.novamusicapp.com');
		}
		else{
			$this->load->view('header');
			$this->load->view('navbar', $data);
	...
	public function save_release($id){
		...
		if($this->form_validation->run()==FALSE){
			...
			$release = $this->Artist_Model->release($id);
			$release_art = $release->release_artwork;
			$uploadData = $this->upload->data();
			$release_artwork =  '/releases/artwork/'.$uploadData['file_name'];

			 if(empty($this->input->post('release_artwork'))){
				$artwork = $release_art;
			}
			else{
...
public function del_notifications($email){
	...
	public function search_artist(){
		...
		$this->form_validation->set_rules('search', 'search', 'required', array('required'=>'No search criteria was given'));
		if($this->form_validation->run()==FALSE){
...
public function edit_profile(){
	...
	public function edit_social(){
		...
		$this->form_validation->set_rules('artist_social_facebook', 'Facebook', 'required');
		$this->form_validation->set_rules('artist_social_instagram', 'Instagram', 'required');
		$this->form_validation->set_rules('artist_social_twitter', 'Twitter', 'required');
		$this->form_validation->set_rules('artist_social_soundcloud', 'Soundcloud', 'required');
		$this->form_validation->set_rules('artist_social_website', 'Website', 'required');
		$this->form_validation->set_rules('artist_social_youtube', 'YouTube', 'required');
	...
	public function do_upload(){
		...
		$this->load->model('Artist_Model');
					...
					//	$data = array('upload_data' => $this->upload->data());
					 ...
					 if(empty('./uploads/'.$_SESSION['artist_social_picture'])){
						$this->session->unset_userdata($data);
						$this->session->set_userdata($data);
