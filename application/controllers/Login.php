...
if ($authenticate==FALSE){
  ...
  $this->session->set_flashdata('acct_not_verified',$not_verified);
  redirect('../Register/verify_code/'.$this->input->post('artist_account_email').'');
}
else {
// session data
$artist_data = array(
    ...
    if($this->form_validation->run()==FALSE){
        ...
        if($reset_pwd){
          ...
          $this->session->set_flashdata('success', $success);
            ...
            $this->load->view('create_pwd', $data);
        }
    }
}
...
private function __send_verification($email, $code){
    ...
    $config['smtp_pass']=''; // Password not shown 
